...
def save_chat_message(
    ...
    :param item_names: 关联的书名列表（可选，支持多书，默认None）
...
def update_message_item_names(ids: List[str], item_names: List[str]) -> int:
    ...
    try:
        ...
        result = mongo_tool.chat_message.update_many(
            ...
            {"$set": {"item_names": item_names}}  # 更新操作：设置新的书名列表
        )
        # 记录更新成功日志，包含更新数量和新的书名
...
if __name__ == "__main__":
    ...
    # 3. 写入带关联书名的用户消息（手动指定ts=1002，测试item_names字段）
    save_chat_message(sid, "user", "《三体》的作者是谁？", item_names=["三体"])
