...
def node_answer_output(state):
    ...
    # 提取引用来源信息（需求4.3/4.4：返回书名、作者名、内容类型、文件名、类别、时长等来源信息）
    sources = _extract_sources_from_docs(state.get("reranked_docs") or [])
    set_task_result(state['session_id'], "sources", sources)
    ...
    if state.get("is_stream"):
        push_to_session(
            ...
            SSEEvent.FINAL,
            {
                ...
                "sources": sources         # 发送引用来源给前端（需求4.3/4.4）
            }
        )
...
def step_2_construct_prompt(state: QueryGraphState) -> str:
    ...
    for i, doc in enumerate(reranked_docs, start=1):
        ...
        score = doc.get("score")
        # ===== 听书领域来源元数据 =====
        dn = doc.get("item_name", "")
        ct = doc.get("content_type", "")
        ft = doc.get("file_title", "")
        ...
        # 听书来源元数据
        if dn:
            meta_parts.append(f"[book={dn}]")
        if ct:
            meta_parts.append(f"[type={ct}]")
        if ft:
            meta_parts.append(f"[file={ft}]")
        if title:
    ...
    # 4. 格式化听书领域字段
    item_names_str = ", ".join(item_names) if item_names else "无指定书籍"
    # 将 scene 转为中文展示
    scene_str = {"recommend": "书籍推荐", "detail": "书籍详情",
                 "retrieval": "内容检索", "notes": "听书笔记",
                 "unknown": "通用问答"}.get(scene, "通用问答")

    # 5. 组装 Prompt（听书化模板，传入 item_names（书名）+ scene 占位符）
...
def _extract_sources_from_docs(docs):
    """
    辅助方法：从检索/重排文档中提取“引用来源”信息（需求4.3/4.4）。
    每个来源包含：书名(item_name)、作者(author_name)、内容类型(content_type)、
    类别(category)、有声书时长(audio_duration)、来源文件名(file_title)。

    去重规则：以 (item_name, author_name, file_title) 组合为唯一键，避免同一本书重复展示；
    同时跳过没有可展示信息的文档。
    """
    if not docs:
        return []
    ...
    for doc in docs:
        if not isinstance(doc, dict):
            continue
        item = (doc.get("item_name") or "").strip()
        author = (doc.get("author_name") or "").strip()
        content_type = (doc.get("content_type") or "").strip()
        category = (doc.get("category") or "").strip()
        audio_duration = (doc.get("audio_duration") or "").strip()
        file_title = (doc.get("file_title") or "").strip()

        # 没有任何可展示信息则跳过
        if not (item or author or content_type or category or audio_duration or file_title):
            continue

        # 组合去重
        key = (item, author, file_title)
        if key in seen:
            continue
        seen.add(key)
    ...
    logger.info(f"引用来源提取完成，共 {len(sources)} 条: {sources}")
    return sources
